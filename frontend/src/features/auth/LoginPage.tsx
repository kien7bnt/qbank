...
export function LoginPage() {
  ...
  const [gsiRendered, setGsiRendered] = useState(true);
  ...
  useEffect(() => {
    ...
    const initGoogleGSI = () => {
      if (window.google?.accounts?.id) {
        ...
        // If not loaded yet, retry once after 500ms; if still unavailable, show fallback button
        timer = setTimeout(() => {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-gray-200/90 bg-white p-5 sm:p-6 shadow-sm space-y-4">
          ...
          <div
            ...
            className={`w-full flex justify-center ${gsiRendered ? 'min-h-[44px]' : 'empty:hidden'}`}
          />

          {/* Fallback Google OAuth Button (chỉ hiển thị khi GSI chưa render hoặc bị chặn) */}
          {!gsiRendered && (
            <button
              type="button"
          ...
          <div className="rounded-xl bg-blue-50/80 border border-blue-100 p-3 text-[11px] text-blue-700 leading-relaxed">
            ✨ <strong>Đăng nhập bằng Google:</strong> Nếu bạn chưa có tài khoản, hệ thống sẽ tự động kích hoạt tài khoản mới và cấp đầy đủ 2 vai trò: <strong>Người dạy</strong> & <strong>Người học</strong>.
          </div>
        </div>
      </div>
    </div>
