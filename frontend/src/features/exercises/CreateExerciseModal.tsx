...
export function CreateExerciseModal({
  ...
  const effectiveCount = isRandom
    ? Math.max(1, Math.min(randomCount, questionIds.length || 1))
    : questionIds.length;
      ...
      footer={
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 w-full">
          ...
          <div className="flex items-center gap-2 justify-end flex-wrap">
            ...
            >
              Lưu Bài Tập ({effectiveCount} câu)
            </Button>
            <Button
            ...
            >
              Lưu & Giao Cho Lớp ({effectiveCount} câu)
            </Button>
          </div>
        </div>
      }
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.checked && (!randomCount || randomCount > questionIds.length)) {
                          setRandomCount(Math.min(30, questionIds.length || 30));
                        }
                      }}
                  ...
                  <div className="space-y-1.5 pt-1.5 border-t border-purple-100">
                    <div>
                      <label className="block text-xs font-semibold text-purple-900 mb-1">
                        Số lượng câu hỏi lấy ngẫu nhiên vào bài tập:
                      </label>
                      <div className="flex items-center gap-2">
                        <Input
                          type="number"
                          min={1}
                          max={questionIds.length || 1000}
                          value={randomCount}
                          onChange={(e) => {
                            const val = Number(e.target.value);
                            const maxVal = questionIds.length > 0 ? questionIds.length : 1000;
                            setRandomCount(val > 0 ? Math.min(maxVal, val) : 1);
                          }}
                          className="w-28 bg-white font-bold text-purple-900 border-purple-300 focus:ring-purple-500 text-xs py-1"
                        />
                        <span className="text-xs text-purple-700">
                          câu {questionIds.length > 0 && <span>(từ <strong>{questionIds.length}</strong> câu đã chọn)</span>}
                        </span>
                      </div>
                    </div>
                    <p className="text-[11px] text-purple-600 italic">
                      Ví dụ: Đã chọn 60 câu, nhập 30 để hệ thống bốc ngẫu nhiên 30 câu vào bài tập.
                    </p>
                  </div>
                ) : (
                  <p className="text-[11px] text-gray-500">
                    Mặc định bài tập sẽ gồm tất cả câu hỏi được chọn. Bật tùy chọn trên nếu muốn bốc ngẫu nhiên số câu ít hơn.
                  </p>
                )}
              </div>
            </div>
