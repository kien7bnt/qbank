...
import { rubricApi, getErrorMessage, getBackendOrigin, getAttachmentUrl } from '@/services/api';
...
export function EssayGradingReviewModal({
  ...
  // Mode: manual (Giáo viên tự chấm) vs ai (Chấm tự động AI theo Rubric)
  const [gradingMode, setGradingMode] = useState<'manual' | 'ai'>('manual');

  // Review Form state
  const [overrideScore, setOverrideScore] = useState<number>(initialScore ?? 0);
  const [teacherComment, setTeacherComment] = useState(initialFeedback ?? '');
  ...
  // Fetch existing essay grading record if any
  ...
  useEffect(() => {
    if (open) {
      if (grading) {
        setOverrideScore(grading.final_score);
        setTeacherComment(grading.final_feedback || '');
      } else if (initialScore !== undefined) {
        setOverrideScore(initialScore);
        setTeacherComment(initialFeedback || '');
      }
    }
  }, [open, grading, initialScore, initialFeedback]);
  ...
  const gradeWithAIMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      qc.invalidateQueries({ queryKey: ['attempt-result'] });
      setOverrideScore(res.data.final_score);
      setTeacherComment(res.data.final_feedback || '');
  ...
  // Manual Teacher Grade Mutation (No AI needed)
  const manualGradeMutation = useMutation({
    mutationFn: () =>
      rubricApi.manualGradeEssay({
        response_id: responseId,
        score: overrideScore,
        feedback: teacherComment.trim(),
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['essay-grading', responseId] });
      qc.invalidateQueries({ queryKey: ['assignment-submissions'] });
      qc.invalidateQueries({ queryKey: ['attempt-result'] });
      toast.success(`Đã lưu kết quả chấm thủ công (${overrideScore}/${maxPoints} đ) thành công!`);
      onOpenChange(false);
    },
    onError: (err) => toast.error(getErrorMessage(err)),
  });

  // Teacher Review / Confirmation Mutation (For existing AI grading)
  const reviewMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['assignment-submissions'] });
      qc.invalidateQueries({ queryKey: ['attempt-result'] });
      toast.success('Đã lưu điểm số của giáo viên thành công!');
      onOpenChange(false);
    },
    onError: (err) => toast.error(getErrorMessage(err)),
  });
        ...
        title={
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-gray-900">
              Chấm bài tự luận {studentName ? `— ${studentName}` : ''}
            </span>
          </div>
        }
        size="xl"
            ...
            <div className="flex items-center gap-2">
              {gradingMode === 'manual' ? (
                ...
                >
                  ...
                  Lưu điểm tự chấm ({overrideScore} / {maxPoints} đ)
                </Button>
              ) : !grading ? (
                <Button
                ...
                <>
                  ...
                  >
                    <CheckCircle2 className="h-4 w-4 mr-1.5" />
                    Lưu điểm ({overrideScore} / {maxPoints} đ)
                  </Button>
                </>
              )}
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-xl border border-blue-200 bg-blue-50/40 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-blue-600 font-bold">Thang điểm: {maxPoints}đ</span>
              </div>
                    ...
                    <button
                      ...
                      onClick={() =>
                        setPreviewAttachment({
                          url: parsedAnswer.attachment!.url,
                          name: parsedAnswer.attachment!.name,
                          type: parsedAnswer.attachment!.type,
                        })
                      }
                      className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-primary-50 hover:bg-primary-100 text-primary-700 text-xs font-semibold transition-colors"
                    >
                      <Eye className="w-3.5 h-3.5" />
                      Xem trực tiếp
                    </button>
                    <a
                      href={getAttachmentUrl(parsedAnswer.attachment.url)}
                      target="_blank"
                      rel="noreferrer"
          ...
          <div className="flex items-center justify-between border-b border-gray-200 pb-2">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>Giáo viên tự chấm thủ công</span>
              </button>
              ...
              >
                ...
                <span>Chấm tự động bằng AI (Rubric)</span>
              </button>
            </div>

            <span className="text-xs text-gray-500 font-medium">
              {gradingMode === 'manual'
                ? '✍️ Nhập điểm trực tiếp theo chuyên môn sư phạm'
                : '🤖 AI đối chiếu tiêu chí và trích xuất dẫn chứng'}
            </span>
          </div>
          ...
          {gradingMode === 'manual' && (
            <div className="space-y-4 p-4 rounded-2xl bg-emerald-50/40 border border-emerald-200">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-xl border border-emerald-100 shadow-2xs">
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-wider text-emerald-800">
                    Điểm số giáo viên chấm
                  </h4>
                  <p className="text-xs text-gray-500 mt-0.5">
                    Nhập điểm chính thức cho câu trả lời của học sinh (Thang điểm: 0 — {maxPoints}đ).
                  </p>
                </div>

                <div className="flex items-center gap-3">
                  <span className="text-xs font-semibold text-gray-600">Cho điểm:</span>
                  <div className="flex items-center gap-1.5">
                    <input
                      ...
                      max={maxPoints}
                      value={overrideScore}
                      onChange={(e) => setOverrideScore(Math.min(maxPoints, Math.max(0, Number(e.target.value))))}
                      className="w-24 px-3 py-1.5 border-2 border-emerald-400 rounded-xl text-lg font-black text-emerald-800 font-mono text-center focus:outline-none focus:border-emerald-600 bg-emerald-50/30"
                    />
                    <span className="text-sm font-bold text-gray-500">/ {maxPoints} đ</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-1.5 bg-white p-4 rounded-xl border border-emerald-100 shadow-2xs">
                <label className="block text-xs font-bold uppercase tracking-wider text-gray-700">
                  Lời phê, nhận xét & hướng dẫn sửa lỗi của Giáo viên
                </label>
                <textarea
                  rows={4}
                  value={teacherComment}
                  onChange={(e) => setTeacherComment(e.target.value)}
                  placeholder="Nhập lời phê, nhận xét bài làm, góp ý chi tiết cho học sinh..."
                  className="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 leading-relaxed"
                />
              </div>
            </div>
          )}
            ...
            <>
              {isLoading ? (
                ...
                <div className="p-8 rounded-2xl border border-dashed border-purple-200 bg-purple-50/50 text-center space-y-3">
                  ...
                  <h4 className="text-sm font-bold text-gray-900">Chưa có kết quả chấm tự luận bằng AI</h4>
                  <p className="text-xs text-gray-500 max-w-md mx-auto">
                    Bấm nút "Bắt đầu chấm AI" để hệ thống tự động phân tích câu trả lời, trích xuất dẫn chứng và tính điểm tất định theo rubric.
                  </p>
                  <Button
                    onClick={() => gradeWithAIMutation.mutate()}
                    loading={gradeWithAIMutation.isPending}
                ...
                <div className="space-y-4">
                  ...
                  {/* Criteria Breakdown */}
                  {grading.criteria_breakdown && grading.criteria_breakdown.length > 0 && (
                    <div className="space-y-3">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-gray-600">
                        Chi tiết đánh giá theo từng tiêu chí Rubric ({grading.criteria_breakdown.length})
                      </h4>
